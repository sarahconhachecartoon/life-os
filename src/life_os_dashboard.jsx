...
const IconCheck  = ({ s = 10 }) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6 9 17l-5-5"/></svg>;
...
const IconClose  = ({ s = 12 }) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M18 6 6 18M6 6l12 12"/></svg>;
...
const IconDrag   = ({ s = 12 }) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><circle cx="9" cy="7" r="1" fill="currentColor"/><circle cx="15" cy="7" r="1" fill="currentColor"/><circle cx="9" cy="12" r="1" fill="currentColor"/><circle cx="15" cy="12" r="1" fill="currentColor"/><circle cx="9" cy="17" r="1" fill="currentColor"/><circle cx="15" cy="17" r="1" fill="currentColor"/></svg>;

// ─── Base ──────────────────────────────────────────────────────────────────────
const Card = ({ children, style = {} }) => (
  <div style={{
    background: '#FFFCF9', borderRadius: 18,
    border: '1px solid rgba(0,0,0,0.052)',
    boxShadow: '0 2px 10px rgba(0,0,0,0.038)',
    padding: '13px 15px', ...style,
  }}>{children}</div>
);

// Pill label — igual que versión original con fondo de color
const Pill = ({ children, color }) => (
  <div style={{
    ...
    padding: '2.5px 11px', marginBottom: 9,
  }}>{children}</div>
);

const Bar = ({ value, accent, h = 3 }) => (
  <div style={{ width: '100%', height: h, borderRadius: h, background: `${accent}20`, overflow: 'hidden' }}>
    <div style={{ width: `${Math.min(value, 100)}%`, height: '100%', background: accent, borderRadius: h, transition: 'width 0.5s' }} />
  </div>
);
...
// ─── BookRow (horizontal compacto para columna derecha) ─────────────────────────
const BookRow = ({ slot, label, book, accent, onEdit, onPageUp }) => {
  const pct = book.totalPages > 0 ? Math.round((book.currentPage / book.totalPages) * 100) : 0;
  return (
    <div style={{ padding: '8px 10px', borderRadius: 10, border: `1px solid ${accent}18`, background: `${accent}06` }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 3 }}>
        <span style={{ fontSize: 8.5, letterSpacing: '0.09em', color: accent, textTransform: 'uppercase', fontWeight: '500' }}>{label}</span>
        <button onClick={onEdit} style={{ background: 'none', border: 'none', cursor: 'pointer', color: `${accent}65`, padding: 0 }}><IconEdit s={10}/></button>
      </div>
      {book.title ? (
        <>
          <div style={{ fontFamily: "'Fraunces', serif", fontStyle: 'italic', fontSize: 12, color: '#3A3230', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{book.title}</div>
          <div style={{ fontSize: 10, color: '#9A8A80', marginBottom: 5, marginTop: 1 }}>{book.author}</div>
          <Bar value={pct} accent={accent} />
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 4 }}>
            <span style={{ fontSize: 10, color: '#9A8A80' }}>p.{book.currentPage}/{book.totalPages} · {pct}%</span>
            <button onClick={onPageUp} style={{ padding: '1px 7px', borderRadius: 5, border: `1px solid ${accent}28`, background: 'transparent', color: accent, fontSize: 10, cursor: 'pointer' }}>+1</button>
          </div>
        </>
      ) : (
        <button onClick={onEdit} style={{ background: 'none', border: 'none', cursor: 'pointer', color: `${accent}50`, fontSize: 11, padding: 0 }}>Añadir libro...</button>
      )}
    </div>
  );
};
...
const TaskModal = ({ task, onSave, onClose, accent }) => {
  ...
  return (
    <>
      ...
      <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', background: '#FDFAF7', borderRadius: 20, border: `1px solid ${accent}30`, boxShadow: '0 20px 48px rgba(0,0,0,0.12)', padding: '26px 28px', width: 370, zIndex: 201 }}>
        ...
        <input autoFocus value={text} onChange={e => setText(e.target.value)} onKeyDown={e => e.key === 'Enter' && save()} placeholder="Descripción..."
          style={{ width: '100%', boxSizing: 'border-box', padding: '9px 13px', borderRadius: 10, border: `1.5px solid ${accent}28`, background: '#FAF7F3', fontSize: 13, color: '#3A3230', fontFamily: "'Inter',sans-serif", outline: 'none', marginBottom: 13 }} />
        <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', marginBottom: 16 }}>
          ...
          <span style={{ fontSize: 13, color: '#5A4A42' }}>Prioritaria (sube al principio)</span>
        </label>
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 10, color: '#9A8A80', letterSpacing: '0.07em', textTransform: 'uppercase', marginBottom: 7 }}>Repetir semanalmente</div>
          <div style={{ display: 'flex', gap: 5 }}>
            {DAYS_SHORT.map((d, i) => (
              <button key={i} onClick={() => toggleDay(i)} style={{ width: 31, height: 31, borderRadius: 8, border: `1.5px solid ${recDays.includes(i) ? accent : '#D8CEC8'}`, background: recDays.includes(i) ? `${accent}20` : 'transparent', color: recDays.includes(i) ? accent : '#9A8A80', fontSize: 11, cursor: 'pointer', transition: 'all 0.15s' }}>{d}</button>
            ))}
          </div>
          {recDays.length > 0 && <div style={{ fontSize: 11, color: accent, marginTop: 6 }}>Repite cada {recDays.map(d => DAYS_ES[d]).join(', ')}</div>}
        </div>
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          <button onClick={onClose} style={{ padding: '8px 17px', borderRadius: 10, border: '1px solid #D8CEC8', background: 'transparent', color: '#9A8A80', fontSize: 13, cursor: 'pointer' }}>Cancelar</button>
          <button onClick={save} style={{ padding: '8px 17px', borderRadius: 10, border: 'none', background: accent, color: '#fff', fontSize: 13, cursor: 'pointer' }}>Guardar</button>
        </div>
      </div>
    </>
  );
...
const BookModal = ({ slot, book, onSave, onClose, accent }) => {
  ...
  return (
    <>
      <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.12)', zIndex: 200, backdropFilter: 'blur(2px)' }} />
      <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', background: '#FDFAF7', borderRadius: 20, border: `1px solid ${accent}30`, boxShadow: '0 20px 48px rgba(0,0,0,0.12)', padding: '26px 28px', width: 350, zIndex: 201 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        ...
        {['title', 'author'].map(k => (
          <input key={k} value={form[k]} onChange={set(k)} placeholder={k === 'title' ? 'Título...' : 'Autor/a...'}
            style={{ width: '100%', boxSizing: 'border-box', padding: '8px 13px', borderRadius: 10, border: `1.5px solid ${accent}25`, background: '#FAF7F3', fontSize: 13, color: '#3A3230', fontFamily: "'Inter',sans-serif", outline: 'none', marginBottom: 9 }} />
        ))}
        <div style={{ display: 'flex', gap: 8, marginBottom: 18 }}>
          {['currentPage', 'totalPages'].map(k => (
            <input key={k} type="number" min="0" value={form[k]} onChange={set(k)} placeholder={k === 'currentPage' ? 'Pág. actual' : 'Total págs.'}
              style={{ flex: 1, padding: '8px 11px', borderRadius: 10, border: `1.5px solid ${accent}25`, background: '#FAF7F3', fontSize: 13, color: '#3A3230', fontFamily: "'Inter',sans-serif", outline: 'none' }} />
          ))}
        </div>
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          <button onClick={onClose} style={{ padding: '8px 17px', borderRadius: 10, border: '1px solid #D8CEC8', background: 'transparent', color: '#9A8A80', fontSize: 13, cursor: 'pointer' }}>Cancelar</button>
          <button onClick={() => { onSave(form); onClose(); }} style={{ padding: '8px 17px', borderRadius: 10, border: 'none', background: accent, color: '#fff', fontSize: 13, cursor: 'pointer' }}>Guardar</button>
        </div>
      </div>
    </>
  );
...
export default function Dashboard({ state, setState, theme }) {
  ...
  // Ordenar: prioritarias primero (preservando orden interno), luego el resto
  const sortedTasks = [
  ...
  // Drag & drop (opera sobre el array original, no el sorted)
  ...
  // ─── RENDER ─────────────────────────────────────────────────────────────────
  // Layout: topbar fijo, luego grid de 3 columnas que ocupa el espacio restante
  // Col izquierda: imagen inspiración (fija, altura completa)
  // Col centro: ToDo arriba + fila hábitos/agenda/menú abajo
  // Col derecha: estado + lectura + notas + monomanía
  ...
  return (
    <div style={{
      ...
      overflow: 'hidden',  // Sin scroll exterior — todo debe caber
    }}>
      ...
      <div style={{
        ...
        <span style={{ fontFamily: "'Fraunces', serif", fontStyle: 'italic', fontSize: 22, color: '#fff' }}>
      ...
      <div style={{
        ...
        <Card style={{ padding: 0, overflow: 'hidden', height: '100%' }}>
          {inspImg ? (
            ...
            <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: `${accent}10`, color: `${accent}55`, fontSize: 12, textAlign: 'center', padding: 16 }}>
              Añade imágenes en Personal
            </div>
          )}
        </Card>

        {/* ── COL 2: To Do + fila inferior ── */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, minHeight: 0, overflow: 'hidden' }}>

          {/* To Do — ocupa ~55% de la altura */}
          <Card style={{ flex: '0 0 55%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 2 }}>
              {sortedTasks.length === 0 && <div style={{ fontSize: 12, color: '#C0B0A8' }}>Sin tareas por hoy</div>}
              {sortedTasks.map((task, i) => (
                ...
                >
                  ...
                  <div onClick={() => toggleTask(task.id)} style={{
                    ...
                    {task.done && <IconCheck s={8}/>}
                  </div>
                  <span style={{ flex: 1, fontSize: 12.5, color: '#3A3230', textDecoration: task.done ? 'line-through' : 'none', wordBreak: 'break-word' }}>
                    {task.text}
                  </span>
                  {task.recurrence && <span style={{ color: `${accent}65`, flexShrink: 0 }} title="Recurrente"><IconRepeat /></span>}
            ...
            {priorityCount > 0 && <div style={{ fontSize: 9.5, color: `${accent}75`, marginTop: 4 }}>{priorityCount}/3 prioritarias</div>}
          </Card>

          {/* Fila inferior: Hábitos + Agenda + Menú */}
          <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1.1fr 1fr 0.7fr', gap: 8, minHeight: 0, overflow: 'hidden' }}>
            ...
            <Card style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
              ...
              <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 3, marginBottom: 8 }}>
                {(state.agenda?.today ?? []).length === 0 && <div style={{ fontSize: 12, color: '#C0B0A8' }}>Sin eventos</div>}
                {(state.agenda?.today ?? []).map(item => (
                  <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '3px 6px', borderRadius: 7, background: `${accent}07` }}>
              ...
              <div style={{ display: 'flex', gap: 5, flexShrink: 0 }}>
                <input type="time" value={agendaInput.time} onChange={e => setAgendaInput(a => ({ ...a, time: e.target.value }))}
                  style={{ width: 66, padding: '5px 7px', borderRadius: 7, border: `1px solid ${accent}22`, background: '#FAF7F3', fontSize: 11, color: '#3A3230', outline: 'none' }} />
                <input value={agendaInput.text} onChange={e => setAgendaInput(a => ({ ...a, text: e.target.value }))} onKeyDown={e => e.key === 'Enter' && addAgenda()} placeholder="Evento..."
                  style={{ flex: 1, padding: '5px 9px', borderRadius: 7, border: `1px solid ${accent}22`, background: '#FAF7F3', fontSize: 11, color: '#3A3230', outline: 'none' }} />
            ...
            <Card style={{ overflowY: 'auto' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                {['desayuno', 'snack', 'almuerzo', 'cena'].map(meal => (
                  <div key={meal} style={{ padding: '4px 7px', borderRadius: 8, background: `${accent}07` }}>
                    <div style={{ fontSize: 8, color: accent, textTransform: 'uppercase', letterSpacing: '0.09em', marginBottom: 1 }}>{meal}</div>
                    <div style={{ fontSize: 11.5, color: state.menu?.[meal] ? '#3A3230' : '#C0B0A8' }}>{state.menu?.[meal] || '—'}</div>
                  </div>
                ))}
              </div>
            </Card>
        ...
        {/* ── COL 3: Estado + Lectura + Notas + Monomanía ── */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, overflowY: 'auto' }}>
          ...
          <Card style={{ flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <div style={{ padding: '6px 10px', borderRadius: 10, background: `${accent}10`, border: `1px solid ${accent}18` }}>
                <div style={{ fontSize: 8.5, color: accent, textTransform: 'uppercase', letterSpacing: '0.09em', marginBottom: 1 }}>Peso</div>
                <div style={{ fontFamily: "'Fraunces', serif", fontSize: 20, color: '#3A3230', lineHeight: 1.1 }}>{latestWeight ? `${latestWeight} kg` : '—'}</div>
              </div>
              <div style={{ padding: '6px 10px', borderRadius: 10, background: cyclePhase ? `${cyclePhase.color}13` : `${accent}10`, border: `1px solid ${cyclePhase ? cyclePhase.color + '25' : accent + '18'}` }}>
                <div style={{ fontSize: 8.5, color: cyclePhase?.color ?? accent, textTransform: 'uppercase', letterSpacing: '0.09em', marginBottom: 1 }}>Ciclo</div>
                <div style={{ fontFamily: "'Fraunces', serif", fontSize: 13, color: '#3A3230' }}>{cycleDay ? `Día ${cycleDay} · ${cyclePhase?.label}` : '—'}</div>
              </div>
              <div>
                <div style={{ fontSize: 8.5, color: accent, textTransform: 'uppercase', letterSpacing: '0.09em', marginBottom: 5 }}>Me siento</div>
                <div style={{ display: 'flex', gap: 3 }}>
                  {MOODS.map(m => (
                    <button key={m.id} onClick={() => setMood(m.id)} style={{ flex: 1, padding: '4px 2px', borderRadius: 7, border: `1.5px solid ${currentMood === m.id ? m.color : m.color + '28'}`, background: currentMood === m.id ? `${m.color}20` : 'transparent', color: m.color, fontSize: 9, cursor: 'pointer', transition: 'all 0.15s' }}>{m.label}</button>
                  ))}
                </div>
              </div>
            </div>
          ...
          {/* Notas */}
          <Card style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 70 }}>
          ...
          <Card style={{ flexShrink: 0 }}>
            ...
            {activeMonomania ? (
              <div>
                ...
                {activeMonomania.notes && <div style={{ fontSize: 11.5, color: '#7A6A62', lineHeight: 1.5 }}>{activeMonomania.notes.slice(0, 90)}{activeMonomania.notes.length > 90 ? '…' : ''}</div>}
              </div>
            ) : (
